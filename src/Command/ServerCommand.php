...
class ServerCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->requestHandler = new RequestHandler(function (Request $request) {
            ...
            $extension = $this->getExtension($resourcePath);

            if($extension == 'php') {
                global $_SERVER;
                $_SERVER['HTTP_HOST'] = $this->host . ':' . $this->port;

                $phpProcess = new PhpProcess($content, $this->documentroot, array('base_url' => $this->host . ':' . $this->port));
                $phpProcess->run();
                $content = $phpProcess->getOutput();
            }
            return $this->getResponseOk($content, $contentType, $extension);
        });
        ...
        try {
            ...
            $this->initialMessage();
            $server = new HttpServer($this->requestHandler);
            $server->start();
        } catch (Exception $e) {
            $output->writeln(
                sprintf(
