...
/**
 * @file
 * Contains \Drupal\Console\Command\Site\StatsCommand.
 */
...
class StatsCommand extends ContainerAwareCommand
{
    ...
    public function configure()
    {
        $this
            ->setName('site:stats')
            ->setDescription($this->trans('commands.site.stats.description'))
            ->setHelp($this->trans('commands.site.stats.help'));
        ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $assets[$this->trans('commands.site.stats.messages.comments')] = $this->getCommentCount();
        $assets[$this->trans('commands.site.stats.messages.vocabulary')] = $this->getTaxonomyVocabularyCount();
        $assets[$this->trans('commands.site.stats.messages.taxonomy-terms')] = $this->getTaxonomyTermCount();
        $assets[$this->trans('commands.site.stats.messages.files')] = $this->getFileCount();
        $assets[$this->trans('commands.site.stats.messages.users')] = $this->getUserCount();
        $assets[$this->trans('commands.site.stats.messages.modules-enabled')] = $this->getModuleCount(true);
        $assets[$this->trans('commands.site.stats.messages.modules-disabled')] = $this->getModuleCount(false);
        $assets[$this->trans('commands.site.stats.messages.themes-enabled')] = $this->getThemeCount(true);
        $assets[$this->trans('commands.site.stats.messages.themes-disabled')] = $this->getThemeCount(false);
    ...
    private function getNodeCount(&$assets)
    {
        ...
        foreach ($nodeTypes as $nodeType) {
            ...
            $key = sprintf(
                $this->trans('commands.site.stats.messages.node-type'),
                $nodeType
            );
