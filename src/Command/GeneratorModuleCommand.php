...
class GeneratorModuleCommand extends GeneratorCommand
{
    ...
    protected function configure()
    {
        $this
          ...
          ->addOption('module-path', '', InputOption::VALUE_REQUIRED,
    ...
    protected function interact(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!$package) {
            $package = $dialog->ask($output,
              $dialog->getQuestion($this->trans('commands.generate.module.questions.package'), 'Other'), 'Other');
        }
        ...
        if (!$core) {
            $core = $dialog->ask($output,
              $dialog->getQuestion($this->trans('commands.generate.module.questions.core'), '8.x'), '8.x');
        }
        $input->setOption('core', $core);

        $controller = $input->getOption('controller');
        if (!$controller && $dialog->askConfirmation($output,
            $dialog->getQuestion($this->trans('commands.generate.module.questions.controller'), 'no', '?'), false)
        ) {
            $controller = true;
        }
        ...
        if (!$dependencies) {
            if ($dialog->askConfirmation($output,
              $dialog->getQuestion($this->trans('commands.generate.module.questions.dependencies'), 'yes', '?'), true)
            ) {
        ...
        if ($controller) {
            $test = $input->getOption('test');
            if (!$test && $dialog->askConfirmation($output,
                $dialog->getQuestion($this->trans('commands.generate.module.questions.test'), 'yes', '?'), true)
            ) {
                $test = true;
            }
        } else {
