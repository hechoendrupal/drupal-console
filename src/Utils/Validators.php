...
class Validators extends Helper implements HelperInterface
{
    ...
    public function validateMachineName($machine_name)
    {
        if (preg_match(self::REGEX_MACHINE_NAME, $machine_name)) {
            ...
            throw new \InvalidArgumentException(sprintf('Machine name "%s" is invalid, it must contain only lowercase letters, numbers and underscores.',
              $machine_name));
        }
    }
    ...
    public function validateSpaces($name)
    {
        ...
        if ($string == $name) {
            ...
            throw new \InvalidArgumentException(sprintf("The name \"%s\" is invalid, spaces between words are not allowed.",
              $name));
        }
    }
